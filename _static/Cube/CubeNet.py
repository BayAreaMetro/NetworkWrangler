...
def export_cubenet_to_csvs(
    file, extra_link_vars=[], extra_node_vars=[], links_csv=None, nodes_csv=None
):
    """
    ...
    script = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "exportHwyfromPy.s"
    )
    ...
    for attempt in range(1, NUM_RETRIES + 1):
        ...
        if hostname not in getCubeHostnames():
            if links_csv == None or nodes_csv == None:
                print(
                    "export_cubenet_to_csvs requires a links_csv and nodes_csv output file if dispatching to {} (temp won't work)".format(
                        CUBE_COMPUTER
                    )
                )
                sys.exit(2)
            ...
            for line in proc.stdout:
                if type(line) == bytes:
                    line = line.decode()  # convert to string, not byetes
                line = line.strip("\r\n")
        ...
        print(
            "...adding variables {}, {}:".format(env["XTRALINKVAR"], env["XTRANODEVAR"])
        )
    ...
    print(
        "Exported network to: {}, {}".format(env["CUBELINK_CSV"], env["CUBENODE_CSV"])
    )
...
def import_cube_nodes_links_from_csvs(
    ...
    if (
        ...
        and links_csv
        and nodes_csv
        and os.path.exists(links_csv)
        and os.path.exists(nodes_csv)
    ):
        pass  # don't need to do anything
    else:
        export_cubenet_to_csvs(
            cubeNetFile,
            extra_link_vars,
            extra_node_vars,
            links_csv=links_csv,
