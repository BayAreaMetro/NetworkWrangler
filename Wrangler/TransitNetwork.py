...
class TransitNetwork(Network):
        ...
        if basenetworkpath and isTiered:
            ...
            for farefile in TransitNetwork.FARE_FILES[self.modelType]:
                ...
                if not os.path.exists(fullfarefile):
                    fullfarefile = os.path.join(
                        basenetworkpath, "transit_fares", farefile
                    )
    ...
    def deleteLine(self, name):
        ...
        if str(type(name)) == str(type(re.compile("."))):
            for idx in range(len(self.lines) - 1, -1, -1):  # go backwards
                ...
                if name.match(self.lines[idx].name):
                    WranglerLogger.debug(
                        "Deleting line {}".format(self.lines[idx].name)
                    )
                    del self.lines[idx]
            return
        ...
        raise NetworkException(
            "deleteLine() didn't understand name argument [{}]".format(name)
        )
    ...
    def parseFileAsSuffix(self, fullfile, suffix, insert_replace):
        ...
        (
            ...
            faresys,
            pts,
        ) = self.parseAndPrintTransitFile(f.read().rstrip("\0"), verbosity=0)
        f.close()
        logstr += self.doMerge(
            fullfile,
            prog,
                ...
                for timeperiod in ["AM", "MD", "PM", "EV", "EA"]:
                    ...
                    try:
                        (
                            vehicletype,
                            cap,
                        ) = TransitNetwork.capacity.getVehicleTypeAndCapacity(
                            linename, timeperiod
                        )
                        if mode in complexDelayModes:
                            (
                                ...
                                delpb,
                                delpa,
                            ) = TransitNetwork.capacity.getComplexDwells(
                                linename, timeperiod
                            )
    ...
    def moveBusesToHovAndExpressLanes(self):
        ...
        WranglerLogger.info(
            "Export these links to {} for debugging".format(hov_unmatched_df_QAQC_FILE)
        )
                ...
                for (a, b) in link_list:
                    ...
                    if (a, b) in links_dict:
                        ...
                        if self.modelType == Network.MODEL_TYPE_TM1:
                            ...
                            ) and (int(links_dict[(a, b)][link_var_names["BRT"]]) != 1):
                                msg = "line {} is running on link {}-{} which has LANES=0 and BRT!=1".format(
                                    line.name, a, b
                                )
