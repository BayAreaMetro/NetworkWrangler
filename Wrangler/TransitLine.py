...
class TransitLine(object):
    ...
    *n* is list of Node objects (see :py:class:`Node`)
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, str):
            ...
            WranglerLogger.error(
                "TransitLine.__eq__ called with other type {}".format(type(other))
            )
            return False
    ...
    def getFreqs(self):
        ...
        try:
            ...
            WranglerLogger.fatal(
                "problem with getFreqs() for {}  self.attr={}".format(
                    self.name, self.attr
                )
            )
    ...
    def extendLine(self, oldnode, newsection, beginning=True):
        ...
        **This is stop-sensitive!**  If *oldnode* has the wrong sign, it will throw an exception.
        ...
        if oldnode_found == False:
            raise NetworkException(
                "extendLine() called but oldnode {} not found in line nodes {}".format(
                    oldnode, self.listNodeIds(ignoreStops=False)
                )
            )
    ...
    def replaceSegment(self, node1, node2, newsection, preserveStopStatus=False):
        ...
        WranglerLogger.debug(
            "replacing segment in {} {}-{} with {}".format(
