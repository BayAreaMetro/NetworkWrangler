...
class TransitAssignmentDataException(Exception):
    pass
...
class TransitAssignmentData:
        ...
        * If *lineLevelAggregateFilename* or *linkLevelAggregateFilename* are passed in, then
        ...
        if tpfactor == "quickboards":
            ...
            for tp in ["AM", "MD", "PM", "EV", "EA"]:
                self.TIMEPERIOD_FACTOR[tp] = (
                    1.0 / TransitLine.HOURS_PER_TIMEPERIOD[modelType][tp]
                )
            ...
            # muni peaking
            muni_peaking = {
                "AM": 0.45,  # 0.39 / 0.85 (Muni peaking factor from 2010 APC / Muni's capacity ratio)
        ...
        if self.timeperiod not in ["AM", "MD", "PM", "EV", "EA"]:
            raise TransitAssignmentDataException(
                "Invalid timeperiod " + str(timeperiod)
            )
    ...
    def readTransitLineGrouping(self, mapfile=None):
        ...
        try:
            ...
            print(
                "couldn't find that workbook %s, yo!  No Groupings used!".format(
                    mapefile
                )
            )
            return lineToGroup
        ...
        while row < sheet.nrows:
            ...
            self.lineToGroup[
                lookupsheet.cell_value(row, 0).encode("utf-8")
            ] = lookupsheet.cell_value(row, 1).encode("utf-8")
            row += 1
        return lineToGroup
    ...
    def initializeFields(self, headerRow=None):
        ...
        self.colnameToCsvIndex = dict(
            (self.csvColnames[idx], idx) for idx in range(len(self.csvColnames))
        )
    ...
    def readTransitAssignmentCsvs(self):
        ...
        for mode in self.MODES:
            if self.modelType == Network.MODEL_TYPE_CHAMP:
                if mode == "WMWVIS":
                    filename = os.path.join(
                        self.assigndir, "VISWMW" + self.timeperiod + ".csv"
                    )
                elif mode[1] == "T":
                    filename = os.path.join(
                        self.assigndir, "NS" + mode + self.timeperiod + ".csv"
                    )
                else:
                    filename = os.path.join(
                        self.assigndir, "SF" + mode + self.timeperiod + ".csv"
                    )
            elif self.modelType == Network.MODEL_TYPE_TM1:
                filename = os.path.join(
                    self.assigndir,
                    "trnlink{}_{}.csv".format(self.timeperiod.lower(), mode),
                )
            ...
            if mode == self.MODES[0]:
                ...
                for row in filereader:
                    ...
                    numrecs += 1
                WranglerLogger.info(
                    "Keeping %d records out of %d" % (numrecs, totalrows)
                )
            ...
            newrownum = (
                0  # row number in the trnAsgnTable,ABNameSeq_List -- rows we're keeping
            )
            ...
            if mode == self.MODES[0]:
                if self.modelType == Network.MODEL_TYPE_CHAMP:
                    indbf = dbfTableReader(
                        os.path.join(self.assigndir, "SFWBW" + self.timeperiod + ".dbf")
                    )
                elif self.modelType == Network.MODEL_TYPE_TM1:
                    indbf = dbfTableReader(
                        os.path.join(
                            self.assigndir,
                            "trnlink{}_{}.dbf".format(self.timeperiod.lower(), mode),
                        )
                    )
            else:
                indbf = None
            ...
            for row in filereader:
                ...
                if mode == self.MODES[0]:
                            ...
                            if self.trnAsgnFields[field][0] in ["u", "b"]:
                                if row[self.colnameToCsvIndex[field]] == "":
                                    ...
                                    self.trnAsgnTable[newrownum][field] = (
                                        float(row[self.colnameToCsvIndex[field]])
                                        * 100.0
                                    )
                                else:
                    ...
                    self.trnAsgnTable[newrownum][
                        "FULLNAME"
                    ] = self.capacity.getFullname(linename, self.timeperiod)

                    try:
                        (vtype, vehcap) = self.capacity.getVehicleTypeAndCapacity(
                            linename, self.timeperiod
                        )

                        self.trnAsgnTable[newrownum]["VEHCAP"] = vehcap
                        self.trnAsgnTable[newrownum]["PERIODCAP"] = (
                            TransitLine.HOURS_PER_TIMEPERIOD[self.modelType][
                                self.timeperiod
                            ]
                            * 60.0
                            * vehcap
                    ...
                    if (
                        self.trnAsgnTable[newrownum]["SYSTEM"] == ""
                        and linename not in warnline
                    ):
                    ...
                    if linename in self.lineToGroup:
                        self.trnAsgnTable[newrownum]["GROUP"] = self.lineToGroup[
                            linename
                        ]
                    else:
        ...
        for row in self.trnAsgnTable:
            ...
            row["LOAD"] = (
                row["AB_VOL"] * tpfactor * row["FREQ"] / (60.0 * row["VEHCAP"])
            )
    ...
    def buildAggregateTable(self):
        ...
        for row in self.trnAsgnTable:
            ...
            self.aggregateTable[rowIndex]["FREQ"] += (
                1 / row["FREQ"]
            )  # combining -- will take reciprocal later
    ...
    def calculateFleetCharacteristics(self):
        ...
        for key in self.trnAsgnTable._index.keys():
            ...
            numveh = (
                TransitLine.HOURS_PER_TIMEPERIOD[self.modelType][self.timeperiod]
                * 60.0
                / record["FREQ"]
            )
    ...
    def writePnrDrivers(self, pnrFileName):
        ...
        self.pnrTable = DataTable(
            fieldNames=self.pnrFields.keys(),
...
if __name__ == "__main__":
    ...
    if False:
        tad1 = TransitAssignmentData(
            directory=r"X:\Projects\GHGReductionCE\2035",
            timeperiod="AM",
            tpfactor="constant",
        )
        print(
            "Test1: vol for MUNKO, 13401-13402, 1) is ",
            tad1.linkVolume("MUNKO", 13401, 13402, 1),
        )
        tad1.writeDbfs(
            asgnFileName=r"X:\lmz\AM_asgn.dbf", aggregateFileName=r"X:\lmz\AM_agg.dbf"
        )
        tad1 = False

    if False:
        tad2 = TransitAssignmentData(
            directory=r"X:\Projects\GHGReductionCE\2035",
            timeperiod="AM",
            tpfactor="constant",
        )
        print(
            "Test2: vol for MUNKO, 13401-13402, 1) is ",
            tad2.linkVolume("MUNKO", 13401, 13402, 1),
        )
        tad2.writeDbfs(
            asgnFileName=r"X:\lmz\AM_asgnF.dbf", aggregateFileName=r"X:\lmz\AM_aggF.dbf"
        )
        tad2 = False

    if True:
        tad3 = TransitAssignmentData(
            timeperiod="AM",
            tpfactor="constant",
        ...
        print(
            "Test3: vol for MUNKO, 13401-13402, 1) is ",
            tad3.linkVolume("MUNKO", 13401, 13402, 1),
        )
        tad3 = False

    if True:
        tad4 = TransitAssignmentData(
            timeperiod="AM",
            tpfactor="constant",
        ...
        print(
            "Test4: vol for MUNKO, 13401-13402, 1) is ",
            tad4.linkVolume("MUNKO", 13401, 13402, 1),
        )
        tad4 = False
