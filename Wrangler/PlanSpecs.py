...
class PlanSpecs:
        ...
        if kwargs:
            ...
            if "tag_override" in kwargs.keys():
                WranglerLogger.debug(
                    "found tag_override: %s" % (kwargs["tag_override"])
                )
                self.tag_override = kwargs["tag_override"]
                if not isinstance(self.tag_override, dict):
                    raise NetworkException(
                        "when passing tag_override, must be dict type"
                    )

        specs = open(
            os.path.join(tempdir, plansubdir, networkdir, "planSpecs.csv"), "r"
        )
        i = 0
        for line in specs:
            i += 1
            if i == 1:
                ...
                try:
                    ...
                    WranglerLogger.debug(
                        "applying project-specific tag %s to project %s within plan %s"
                        % (project_tag, project_name, self.plan_name)
                    )
                except:
                    project_tag = tag
                    WranglerLogger.debug(
                        "applying general tag %s to project %s within plan %s"
                        % (project_tag, project_name, self.plan_name)
                    )
                ...
                try:
                    ...
                    self.projectdict[project_name][
                        "modelyear"
                    ] = None  # WranglerLogger.debug(
                    # WranglerLogger.debug('project %s: MODELYEAR error: %s' % (project_name, e))
                try:
                    self.projectdict[project_name]["tag"] = kwargs["tag_override"][
                        project_name
                    ]
                ...
                if head:
                    applied_SHA1 = self.network.cloneProject(
                        networkdir=head,
                        projectsubdir=tail,
                        tag=project_tag,
                        projtype=projType,
                    ...
                    (
                        ...
                        gitdir,
                        projectsubdir,
                    ) = self.network.getClonedProjectArgs(head, tail, projType, tempdir)
                    self.projectdict[project_name][
                        "nettypes"
                    ] = self.network.getNetTypes(tempdir, head, tail)
                else:
                    applied_SHA1 = self.network.cloneProject(
                    ...
                    (
                        ...
                        gitdir,
                        projectsubdir,
                    ) = self.network.getClonedProjectArgs(
                        project_name, None, projType, tempdir
                    )
                    self.projectdict[project_name][
                        "nettypes"
                    ] = self.network.getNetTypes(tempdir, project_name)
                self.projectdict[project_name]["year"] = self.network.getAttr(
                    "year", parentdir, networkdir, gitdir, projectsubdir
                )
    ...
    def projectAsDict(self, project_name):
        ...
        if "modelyear" in self.projectdict[project_name].keys():
            projDict["kwargs"]["modelyear"] = self.projectdict[project_name][
                "modelyear"
            ]
