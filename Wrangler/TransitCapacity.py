...
class TransitCapacity:
    ...
    def readTransitVehicleToCapacity(
        self, directory=".", filename="transitVehicleToCapacity.csv"
    ):
        """
        ...
        for line in lines:
            ...
            if len(tokens) > 4:
                ...
                self.vehicleTypeToDelays[vtype][TransitCapacity.DELAY_SIMPLE] = float(
                    tokens[4]
                )
                self.vehicleTypeToDelays[vtype][TransitCapacity.DELAY_CONST] = float(
                    tokens[5]
                )
                self.vehicleTypeToDelays[vtype][TransitCapacity.DELAY_PERBOARD] = float(
                    tokens[6]
                )
                self.vehicleTypeToDelays[vtype][
    ...
    def writeTransitVehicleToCapacity(
        self, directory=".", filename="transitVehicleToCapacity.csv"
    ):
        """
        ...
        f.write(
            "VehicleType,100%Capacity,85%Capacity,VehicleCategory,SimpleDelayPerStop,ConstDelayPerStop,DelayPerBoard,DelayPerAlight\n"
        )
        for vehicleType in sorted(self.vehicleTypeToCapacity.keys()):
            ...
            if vehicleType in self.vehicleTypeToDelays:
                f.write(
                    ",%s" % vehicleType
                )  # this is supposed to be vehicle category but we didn't keep that around since we don't use it
                f.write(
                    ",%.3f,%.3f,%.3f,%.3f"
                    % (
                        self.vehicleTypeToDelays[vehicleType][
                            TransitCapacity.DELAY_SIMPLE
                        ],
                        self.vehicleTypeToDelays[vehicleType][
                            TransitCapacity.DELAY_CONST
                        ],
                        self.vehicleTypeToDelays[vehicleType][
                            TransitCapacity.DELAY_PERBOARD
                        ],
                        self.vehicleTypeToDelays[vehicleType][
                            TransitCapacity.DELAY_PERALIGHT
                        ],
    ...
    def readTransitLineToVehicle(
        self, directory=".", filename="transitLineToVehicle.csv"
    ):
        """
        ...
        for (
            ...
            self.linenameToAttributes[name.upper()] = [
                system,
                fullLineName,
                vehicleTypeAM,
                vehicleTypePM,
    ...
    def writeTransitLineToVehicle(
        ...
        f = open(os.path.join(directory, filename), "w")
        f.write(
            "Name,System,Stripped,Line,FullLineName,AM VehicleType,PM VehicleType,OP Vehicle Type\n"
        )
        for linename in sorted(self.linenameToAttributes.keys()):
            f.write(linename + ",")
            f.write(
                self.linenameToAttributes[linename][TransitCapacity.ATTR_SYSTEM] + ","
            )
            f.write(self.linenameToSimple[linename][0] + ",")  # stripped
            f.write(self.linenameToSimple[linename][1] + ",")  # simplename
            f.write(
                self.linenameToAttributes[linename][TransitCapacity.ATTR_FULLNAME] + ","
            )
            f.write(
                self.linenameToAttributes[linename][TransitCapacity.ATTR_AMVEHTYPE]
                + ","
            )
            f.write(
                self.linenameToAttributes[linename][TransitCapacity.ATTR_PMVEHTYPE]
                + ","
            )
            f.write(
                self.linenameToAttributes[linename][TransitCapacity.ATTR_OPVEHTYPE]
                + "\n"
            )
        f.close()

    def readTransitPrefixToVehicle(
        self, directory=".", filename="transitPrefixToVehicle.csv"
    ):
        """
    ...
    def writeTransitPrefixToVehicle(
        self, directory=".", filename="transitPrefixToVehicle.csv"
    ):
        """
        Writes it out in the same format
    ...
    def getSystemAndVehicleType(self, linename, timeperiod):
        ...
        if linenameU in self.linenameToAttributes:
            return (
                self.linenameToAttributes[linenameU][TransitCapacity.ATTR_SYSTEM],
                self.linenameToAttributes[linenameU][
                    TransitCapacity.TIMEPERIOD_TO_VEHTYPIDX[timeperiod]
                ],
            )

        if linename[:4] in self.prefixToVehicleType:
            return (
                self.prefixToVehicleType[linenameU[:4]][0],
                self.prefixToVehicleType[linenameU[:4]][1],
            )

        if linename[:3] in self.prefixToVehicleType:
            return (
                self.prefixToVehicleType[linenameU[:3]][0],
                self.prefixToVehicleType[linenameU[:3]][1],
            )
    ...
    def getVehicleTypeAndCapacity(self, linename, timeperiod):
        ...
        if vehicleType not in self.vehicleTypeToCapacity:
            raise NetworkException(
                "Vehicle type [%s] of system [%s] characteristics unknown; line name = [%s]"
                % (vehicleType, system, linename.upper())
            )
    ...
    def getSimpleDwell(self, linename, timeperiod):
        ...
        if vehicleType not in self.vehicleTypeToDelays:
            raise NetworkException(
                "Vehicle type [%s] of system [%s] simple dwell unknown; line name = [%s]"
                % (vehicleType, system, linename.upper())
            )
    ...
    def getComplexDwells(self, linename, timeperiod):
        ...
        if vehicleType not in self.vehicleTypeToDelays:
            raise NetworkException(
                "Vehicle type [%s] of system [%s] complex dwell unknown; line name = [%s]"
                % (vehicleType, system, linename.upper())
            )
    ...
    def addLinenameFromTemplate(self, newLine, templateLine):
        ...
        if templateLine not in self.linenameToAttributes:
            raise NetworkException(
                "addLinename with unknown templateLine %s for %s"
                % (templateLine, newLine)
            )

        self.linenameToAttributes[newLine] = copy.deepcopy(
            self.linenameToAttributes[templateLine]
        )
        self.linenameToSimple[newLine] = copy.deepcopy(
            self.linenameToSimple[templateLine]
        )

    def addLineName(
        self, newLine, system, fullname, vehicletype_AM, vehicletype_PM, vehicletype_OP
    ):
        """
        Adds a new line with the given vehicle type information
        """
        self.linenameToAttributes[newLine] = [
            system,
            fullname,
            vehicletype_AM,
            vehicletype_PM,
    ...
    def setVehicleTypeForPrefix(self, prefix, system, vehicleType):
        ...
        if vehicleType not in self.vehicleTypeToCapacity:
            WranglerLogger.warn(
                "Setting vehicle type for prefix {} but vehicleType {} unknown"
                % (prefix, vehicleType)
            )

        if prefix in self.prefixToVehicleType.keys():
            WranglerLogger.debug(
                "Overwriting system/vehicle type for prefix {}.  Was {}/{}, setting to {}/{}".format(
                    prefix,
                    self.prefixToVehicleType[prefix][0],
                    self.prefixToVehicleType[prefix][1],
                    system,
                    vehicleType,
                )
            )
    ...
    def setVehicleTypes(
        ...
        *linename* is a string; pass *lineNameIsRegex* to interpret it as a regex
        ...
        if vehicleType_AM not in self.vehicleTypeToCapacity:
            WranglerLogger.warn(
                "Setting vehicle type for line %s but vehicleType %s unknown"
                % (linename, vehicleType_AM)
            )
        if vehicleType_PM not in self.vehicleTypeToCapacity:
            WranglerLogger.warn(
                "Setting vehicle type for line %s but vehicleType %s unknown"
                % (linename, vehicleType_PM)
            )
        if vehicleType_OP not in self.vehicleTypeToCapacity:
            WranglerLogger.warn(
                "Setting vehicle type for line %s but vehicleType %s unknown"
                % (linename, vehicleType_OP)
            )
        ...
        if lineNameIsRegex:
            ...
            for name in self.linenameToAttributes.keys():
                if re.search(linename_re, name):
                    self.linenameToAttributes[name][
                        TransitCapacity.ATTR_AMVEHTYPE
                    ] = vehicleType_AM
                    self.linenameToAttributes[name][
                        TransitCapacity.ATTR_PMVEHTYPE
                    ] = vehicleType_PM
                    self.linenameToAttributes[name][
                        TransitCapacity.ATTR_OPVEHTYPE
                    ] = vehicleType_OP
                    num_updated += 1

            if num_updated == 0:
                WranglerLogger.warn(
                    "setVehicleTypes called with lineNameIsRegex and no matching configuration for line name {}".format(
                        linename
                    )
                )
        else:
            if linename.upper() not in self.linenameToAttributes:
                raise NetworkException(
                    "TransitCapacity: setAllVehicleTypes for unknown linename %s"
                    % linename
                )

            self.linenameToAttributes[linename.upper()][
                TransitCapacity.ATTR_AMVEHTYPE
            ] = vehicleType_AM
            self.linenameToAttributes[linename.upper()][
                TransitCapacity.ATTR_PMVEHTYPE
