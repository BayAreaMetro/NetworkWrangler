...
class HighwayNetwork(Network):
    ...
    def getCubeHostnames():
        ...
        """
        # got them already
        if HighwayNetwork.cube_hostnames:
        ...
        for line in f:
            if line[0] == "#":
                continue
            HighwayNetwork.cube_hostnames.append(
                line.split()[0]
            )  # use the first token of non-comment lines
        f.close()
        return HighwayNetwork.cube_hostnames
    ...
    def __init__(
        ...
        *hwyspecs*, if passed in, should be an instance of :py:class:`HwySpecsRTP`.  It
        ...
        if isTiered:
            ...
            self.cloneAndApplyProject(
                networkdir=basenetworkpath, tempdir=tempdir, projtype="seed", tag=tag
            )
    ...
    def applyBasenetwork(self, parentdir, networkdir, gitdir, tierNetworkName):
        ...
        tierNetwork = os.path.join(
            parentdir,
            networkdir,
            tierNetworkName if tierNetworkName else "FREEFLOW.net",
        )
        ...
        for filename in ["turnsam.pen", "turnspm.pen", "turnsop.pen", "tolls.csv"]:
            try:
                ...
                WranglerLogger.warn(
                    "Couldn't find file {} -- Touching an empty file".format(filename)
                )
                # touch a blank file
        ...
        for filename in [
            ...
            if to_suffix:
                ...
                WranglerLogger.debug(
                    "Copying {:20} to {}{}".format(filename, filename, suffix)
                )
            else:
                shutil.copy2(src="{}{}".format(filename, suffix), dst=filename)
                WranglerLogger.debug(
                    "Copying {:20} to {}".format(filename + suffix, filename)
                )
    ...
    def applyProject(self, parentdir, networkdir, gitdir, projectsubdir=None, **kwargs):
        ...
        if projectsubdir:
            ...
            applyScript = "apply.s"
            descfilename = os.path.join(
                parentdir, networkdir, projectsubdir, "desc.txt"
            )
            turnsfilename = os.path.join(
                parentdir, networkdir, projectsubdir, "turns.pen"
            )
            tollsfilename = os.path.join(
                parentdir, networkdir, projectsubdir, "tolls.csv"
            )
        else:
        ...
        for attempt in range(1, NUM_RETRIES + 1):
            ...
            if hostname not in HighwayNetwork.getCubeHostnames():
                ...
                f.close()
                (cuberet, cubeStdout, cubeStderr) = self._runAndLog(
                    "Y:/champ/util/bin/dispatch.bat runtpp_dispatch.tmp taraval",
                    run_dir=applyDir,
                ...
                (cuberet, cubeStdout, cubeStderr) = self._runAndLog(
                    cmd="runtpp " + applyScript, run_dir=applyDir, env=kwargs
                )
            ...
            for line in cubeStdout:
                line = line.rstrip()
                if re.match(nodemerge, line):
                    continue
                if re.match(linkmerge, line):
                    continue
                if line == "RUNTPP: Licensing error":
                    license_error = True
            ...
            if license_error:
                ...
                if attempt == NUM_RETRIES:
                    ...
                    raise NetworkException(
                        "HighwayNetwork applyProject failed from Licensing error"
                    )
            ...
            if cuberet != 0 and cuberet != 1:
                ...
                WranglerLogger.fatal(
                    "FAIL! Project: {}  cuberet={}".format(applyScript, cuberet)
                )
                raise NetworkException(
                    "HighwayNetwork applyProject failed; see log file"
                )
        ...
        county = None
        if (
            networkdir == self.hwyspecsdir
            and self.hwyspecs
            and projectsubdir in self.hwyspecs.projectdict
        ):
            year = self.hwyspecs.projectdict[projectsubdir]["MOD YEAR"]
            county = self.hwyspecs.projectdict[projectsubdir]["County"]
            desc = (
                self.hwyspecs.projectdict[projectsubdir]["Facility"]
                + ", "
    ...
    def mergeTolls(self, tollsfile, newtollsfile):
        ...
        if fieldnames == new_fieldnames:
            ...
            len(new_fieldnames) < len(fieldnames)
            and fieldnames[: len(new_fieldnames)] == new_fieldnames
        ):
            # ok -- some columns at end can be blank
            pass
        else:
            raise NetworkException(
                "Toll file {} has different fieldnames ({}) than expected ({})".format(
                    newtollsfile, new_fieldnames, fieldnames
                )
            )
        ...
        # write it out
        tolls = open(
            tollsfile, mode="w", newline=""
        )  # newline arg passed because of https://docs.python.org/3/library/csv.html#id3
    ...
    def validateTurnPens(
        self, CubeNetFile, turnPenReportFile=None, suggestCorrectLink=True
    ):
        import Cube
        ...
        if turnPenReportFile:
            ...
            outfile.write(
                "file,old_from,old_through,old_to,on_street,at_street,new_from,new_through,new_to,note\n"
            )
            ...
            for line in f:
                ...
                if m:
                    ...
                    if not (fr_node, th_node) in links_dict:
                        WranglerLogger.debug(
                            "HighwayNetwork.validateTurnPens: (%d, %d) not in the roadway network for %s (%d, %d, %d)"
                            % (fr_node, th_node, file_name, fr_node, th_node, to_node)
                        )
                    ...
                    if not (th_node, to_node) in links_dict:
                        WranglerLogger.debug(
                            "HighwayNetwork.validateTurnPens: (%d, %d) not in the roadway network for %s (%d, %d, %d)"
                            % (th_node, to_node, file_name, fr_node, th_node, to_node)
                        )
                    ...
                    if new_th != None:
                        ...
                        outfile.write(
                            ...
                            % (
                                ...
                                from_street,
                                to_street,
                                new_fr if new_fr else -1,
                                new_th,
                                new_to if new_to else -1,
                            )
                        )
    ...
    def write(
        ...
        if not suppressValidation:
            self.validateTurnPens(netfile, "turnPenValidations.csv")
