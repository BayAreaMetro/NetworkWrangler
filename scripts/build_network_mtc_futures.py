...
def getProjectYear(PROJECTS, my_proj, netmode):
    ...
    for year in PROJECTS.keys():
        if my_proj in PROJECTS[year][netmode]:
            return "{}.{}.{:0>2d}".format(
                year, netmode, (PROJECTS[year][netmode].index(my_proj) + 1)
            )
    return -1
...
def checkRequirements(REQUIREMENTS, PROJECTS, req_type="prereq"):
    ...
    for netmode in REQUIREMENTS.keys():
        for project in REQUIREMENTS[netmode].keys():
            ...
            if project_year == -1:
                Wrangler.WranglerLogger.warn(
                    "Cannot find the %s project %s to check its requirements".format(
                        netmode, project
                    )
                )
                continue  # raise?
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--analysis",
        choices=["Round1", "Round2", "PPA", "PPA_NoSLR"],
        help="Specify which set of analysis are relevant for these networks.",
        default="Round1",
    )
    parser.add_argument(
        "net_spec",
    ...
    parser.add_argument(
        "future",
        choices=["CleanAndGreen", "RisingTides", "BackToTheFuture"],
        help="Specify which Future Scenario for which to create networks",
    )
    ...
    for YEAR in NETWORK_PROJECTS.keys():
        ...
        for netmode in NET_MODES:
            ...
            for project in projects_for_year[netmode]:
                ...
                if (project_name == "Earthquake") and (
                    (
                        PROJECT == "FU1"
                        and args.future in ["CleanAndGreen", "BackToTheFuture"]
                    )
                    or (PROJECT == "FU2")
                ):
                    ...
                    networks_without_earthquake[netmode] = copy.deepcopy(
                        networks[netmode]
                    )
                    if netmode == "hwy":
                        shutil.copyfile(
