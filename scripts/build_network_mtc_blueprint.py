...
def checkRequirements(REQUIREMENTS, PROJECTS, req_type="prereq"):
    ...
    for netmode in REQUIREMENTS.keys():
        for project in REQUIREMENTS[netmode].keys():
            ...
            if project_year == -1:
                Wrangler.WranglerLogger.warning(
                    "Cannot find the {} project {} to check its requirements".format(
                        netmode, project
                    )
                )
                continue  # raise?
            ...
            Wrangler.WranglerLogger.info(
                "Checking {} project {} ({}) for {}".format(
                    netmode, project, project_year, req_type
                )
            )
                ...
                for req_proj in req_proj_list:
                    ...
                    if req_type == "prereq":
                        ...
                        if req_project_year > project_year:
                            ...
                            Wrangler.WranglerLogger.warning(
                                "required project year {} > project year {}".format(
                                    req_project_year, project_year
                                )
                            )
...
def writeRequirements(REQUIREMENTS, PROJECTS, req_type="prereq"):
    ...
    Wrangler.WranglerLogger.info(
        "    Year    {:50}     {:50} Year".format(
            "Project", print_req + " " + "Project"
        )
    )
...
def getProjectAttributes(project):
    ...
    if type(project) == type({"this is": "a dictionary"}):
        ...
        if "kwargs" in project:
            kwargs = project["kwargs"]
...
def preCheckRequirementsForAllProjects(networks, continue_on_warning):
    ...
    for netmode in NET_MODES:
        ...
        for model_year in NETWORK_PROJECTS.keys():
            for project in NETWORK_PROJECTS[model_year][netmode]:
                ...
                if BUILD_MODE == "test" and type(TEST_PROJECTS) == type(["List"]):
                    if project_name in TEST_PROJECTS:
                        Wrangler.WranglerLogger.debug(
                            "Skipping tag [%s] because test mode and [%s] is in TEST_PROJECTS"
                            % (TAG, project_name)
                        )
                        tag = None
                ...
                if head:
                    cloned_SHA1 = networks[netmode].cloneProject(
                        networkdir=head,
                        projectsubdir=tail,
                        tag=tag,
                        projtype=projType,
                    ...
                    (prereqs, coreqs, conflicts) = networks[netmode].getReqs(
                        networkdir=head,
                        projectsubdir=tail,
                        tag=tag,
                        projtype=projType,
                ...
                cmd_dir = os.path.join(
                    Wrangler.Network.NETWORK_BASE_DIR, join_subdir, project_name
                )
                (retcode, retStdout, retStderr) = networks[netmode]._runAndLog(
                    cmd, run_dir=cmd_dir
                )
                # Wrangler.WranglerLogger.debug("results of [%s]: %s %s %s" % (cmd, str(retcode), str(retStdout), str(retStderr)))
                if (
                    retcode != 0
                ):  # this shouldn't happen -- wouldn't cloneAndApply have failed?
                    Wrangler.WranglerLogger.fatal(
                        "Couldn't run cmd [%s] in [%s]: stdout=[%s] stderr=[%s]"
                ...
                if cloned_SHA1 != head_SHA1:
                    Wrangler.WranglerLogger.warning(
                        "Using non-head version of project of %s" % project_name
                    )
                    Wrangler.WranglerLogger.warning(
                        "  Applying version [%s], Head is [%s]"
                        % (cloned_SHA1, head_SHA1)
                    )
                    ...
                    Wrangler.WranglerLogger.warning(
                        "  The following commits are not included:"
                    )
                    ...
                    if continue_on_warning:
                        Wrangler.WranglerLogger.warning(
                            "Continuing (continue_on_warning)"
                        )
                ...
                # find out if the project is stale
                    ...
                    if applied_commit_age > datetime.timedelta(days=365 * STALE_YEARS):
                        Wrangler.WranglerLogger.warning(
                            "  This project was last updated %.1f years ago (over %d), on %s"
                            % (
                                applied_commit_age.days / 365.0,
                                STALE_YEARS,
                                applied_commit_date.strftime("%x"),
                            )
                        )
                        if continue_on_warning:
                            Wrangler.WranglerLogger.warning(
                ...
                if len(prereqs) > 0:
                    PRE_REQS[netmode][project_name] = prereqs
                if len(coreqs) > 0:
                    CO_REQS[netmode][project_name] = coreqs
                if len(conflicts) > 0:
                    CONFLICTS[netmode][project_name] = conflicts
    ...
    # Check prereqs
    (PRE_REQS, allPrereqsFound) = checkRequirements(
        PRE_REQS, NETWORK_PROJECTS, req_type="prereq"
    )
    if len(PRE_REQS["trn"]) > 0 or len(PRE_REQS["hwy"]) > 0:
        writeRequirements(PRE_REQS, NETWORK_PROJECTS, req_type="prereq")
        if allPrereqsFound:
            Wrangler.WranglerLogger.debug(
                "All PRE-REQUISITES were found. Are the PRE-REQUISITES matches correct? (y/n)"
            )
        else:
            Wrangler.WranglerLogger.debug(
                "!!!WARNING!!! Some PRE-REQUISITES were not found or ordered correctly.  Continue anyway? (y/n)"
            )
        response = input("")
        Wrangler.WranglerLogger.debug("  response = [%s]" % response)
        if response.strip().lower() not in ["y", "yes"]:
    ...
    # Check coreqs
    (CO_REQS, allCoreqsFound) = checkRequirements(
        CO_REQS, NETWORK_PROJECTS, req_type="coreq"
    )
    if len(CO_REQS["trn"]) > 0 or len(CO_REQS["hwy"]) > 0:
        writeRequirements(CO_REQS, NETWORK_PROJECTS, req_type="coreq")
        if allCoreqsFound:
            Wrangler.WranglerLogger.debug(
                "All CO-REQUISITES were found. Are the CO-REQUISITE matches correct? (y/n)"
            )
        else:
            Wrangler.WranglerLogger.debug(
                "!!!WARNING!!! Some CO-REQUISITES were not found.  Continue anyway? (y/n)"
            )
        response = input("")
        Wrangler.WranglerLogger.debug("  response = [%s]" % response)
        if response.strip().lower() not in ["y", "yes"]:
    ...
    # Check conflicts
    (CONFLICTS, anyConflictFound) = checkRequirements(
        CONFLICTS, NETWORK_PROJECTS, req_type="conflict"
    )
    if len(CONFLICTS["trn"]) > 0 or len(CONFLICTS["hwy"]) > 0:
        writeRequirements(CONFLICTS, NETWORK_PROJECTS, "conflict")
        if anyConflictFound:
            Wrangler.WranglerLogger.debug(
                "!!!WARNING!!! Conflicting projects were found.  Continue anyway? (y/n)"
            )
        else:
            Wrangler.WranglerLogger.debug(
                "No conflicting projects were found. Enter 'y' to continue."
            )
        response = input("")
        Wrangler.WranglerLogger.debug("  response = [%s]" % response)
        if response.strip().lower() not in ["y", "yes"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--configword", help="optional word for network specification script"
    )
    parser.add_argument(
        "--model_type",
        choices=[Wrangler.Network.MODEL_TYPE_TM1, Wrangler.Network.MODEL_TYPE_TM2],
        default=Wrangler.Network.MODEL_TYPE_TM1,
    )
    parser.add_argument(
        "--continue_on_warning",
        help="Don't prompt the user to continue if there are warnings; just warn and continue",
        action="store_true",
    )
    parser.add_argument(
        "--skip_precheck_requirements",
        help="Don't precheck network requirements, stale projects, non-HEAD projects, etc",
        action="store_true",
    )
    parser.add_argument(
        "net_spec",
        metavar="network_specification.py",
        help="Script which defines required variables indicating how to build the network",
    )
    parser.add_argument(
        "netvariant",
        choices=["Baseline", "Blueprint", "Alt1", "Alt2", "NextGenFwy", "TIP2023"],
    ...
    LOG_FILENAME = "build%snetwork_%s_%s_%s.info.LOG" % (
        "TEST" if BUILD_MODE == "test" else "",
        PROJECT,
        NET_VARIANT,
        NOW,
    )
    Wrangler.setupLogging(
        os.path.join("BlueprintNetworks", LOG_FILENAME),
    ...
    if TRANSIT_CAPACITY_DIR:
        Wrangler.TransitNetwork.capacity = Wrangler.TransitCapacity(
            directory=TRANSIT_CAPACITY_DIR
        )
    ...
    if args.skip_precheck_requirements:
        Wrangler.WranglerLogger.info(
            "skip_precheck_requirements passed so skipping preCheckRequirementsForAllProjects()"
        )
    else:
    ...
    for YEAR in NETWORK_PROJECTS.keys():
        ...
        for netmode in NET_MODES:
            Wrangler.WranglerLogger.info(
                "Building {} {} networks".format(YEAR, netmode)
            )
            ...
            for project in projects_for_year[netmode]:
                ...
                Wrangler.WranglerLogger.info(
                    "Applying project [{}] of type [{}] with tag [{}] and kwargs[{}]".format(
                ...
                (parentdir, networkdir, gitdir, projectsubdir) = networks[
                    netmode
                ].getClonedProjectArgs(project_name, None, projType, TEMP_SUBDIR)

                applied_SHA1 = networks[netmode].applyProject(
                    parentdir, networkdir, gitdir, projectsubdir, **kwargs
                )
                appliedcount += 1

                # if hwy project has set_capclass override, copy it to set_capclass/apply.s
                set_capclass_override = os.path.join(
                    TEMP_SUBDIR, project_name, "set_capclass.job"
                )
                if os.path.exists(set_capclass_override):
                    ...
                    Wrangler.WranglerLogger.info(
                        "Copied override {} to {}".format(
                            set_capclass_override, dest_file
                        )
                    )

        if appliedcount == 0:
            Wrangler.WranglerLogger.info(
                "No applied projects for this year -- skipping output"
            )
            continue

        if NET_VARIANT != "Baseline" and YEAR == 2015:
            Wrangler.WranglerLogger.info(
                "Blueprint 2015 == Baseline 2015 -- skipping output"
            )
            continue
        ...
        if NET_VARIANT != "Baseline" or YEAR < 2035:
            ...
            hwypath = os.path.join(
                "..",
                "BlueprintNetworks",
                "net_{}_{}".format(YEAR, NET_VARIANT),
                HWY_SUBDIR,
            )
            if not os.path.exists(hwypath):
                os.makedirs(hwypath)
            trnpath = os.path.join(
                "..",
                "BlueprintNetworks",
                "net_{}_{}".format(YEAR, NET_VARIANT),
                TRN_SUBDIR,
            )
            if not os.path.exists(trnpath):
                os.makedirs(trnpath)

            # apply set_capclass before writing any hwy network
            applied_SHA1 = networks["hwy"].applyProject(
            ...
            )  # MTC doesn't have turn penalties
            ...
            networks["trn"].write(
                ...
                cubeNetFileForValidation=os.path.join(
                    os.path.abspath(hwypath), HWY_NET_NAME
                ),
            )

            # Write the transit capacity configuration
            Wrangler.TransitNetwork.capacity.writeTransitVehicleToCapacity(
                directory=trnpath
            )
            Wrangler.TransitNetwork.capacity.writeTransitLineToVehicle(
                directory=trnpath
            )
            Wrangler.TransitNetwork.capacity.writeTransitPrefixToVehicle(
                directory=trnpath
            )
        ...
        if NET_VARIANT == "Baseline" and YEAR >= 2035:
            ...
            if YEAR >= 2035 and YEAR < 2050:
                BP_SLR_PROJECT = {
                    "name": "BP_Sea_Level_Rise_Inundation",
                    "kwargs": {"MODELYEAR": "2035"},
                }
            if YEAR == 2050:
                BP_SLR_PROJECT = {
                    "name": "BP_Sea_Level_Rise_Inundation",
                    "kwargs": {"MODELYEAR": "2050"},
                }
            ...
            for netmode in NET_MODES:
                (project_name, projType, tag, kwargs) = getProjectAttributes(
                    BP_SLR_PROJECT
                )
                # Wrangler.WranglerLogger.debug("BP SLR Project {} has project_name=[{}] projType=[{}] tag=[{}] kwargs=[{}]".format(BP_SLR_PROJECT,
                #                                project_name, projType, tag, kwargs))
                applied_SHA1 = None
                copyloned_SHA1 = networks_bp_baseline[netmode].cloneProject(
                    networkdir=project_name,
                    tag=tag,
                    projtype=projType,
                    tempdir=TEMP_SUBDIR,
                ...
                (parentdir, networkdir, gitdir, projectsubdir) = networks_bp_baseline[
                    netmode
                ].getClonedProjectArgs(project_name, None, projType, TEMP_SUBDIR)
                applied_SHA1 = networks_bp_baseline[netmode].applyProject(
                    parentdir, networkdir, gitdir, projectsubdir, **kwargs
                )
            ...
            applied_SHA1 = networks_bp_baseline["hwy"].applyProject(
                parentdir=TEMP_SUBDIR,
                networkdir=SET_CAPCLASS,
                gitdir=os.path.join(TEMP_SUBDIR, SET_CAPCLASS),
            )

            networks_bp_baseline["hwy"].write(
                path=hwypath,
                name=HWY_NET_NAME,
                suppressQuery=True,
                suppressValidation=True,
            ...
            networks_bp_baseline["trn"].write(
                path=trnpath,
                name="transitLines",
                writeEmptyFiles=False,
                suppressQuery=True if BUILD_MODE == "test" else False,
    ...
    Wrangler.WranglerLogger.debug(
        "Successfully completed running %s" % os.path.abspath(__file__)
    )
