...
NGF_PROJECTS = {
    ...
    # Pathway 8: 2035 Dual Express Lanes (no capacity increase, 2 EL lanes if total number of lanes <= 3). Plus HOV conversion.
    # This pathway is essentially Pathway 6A plus HOV conversion. 
    # https://app.asana.com/0/1203644633064654/1208347476026379/f
    'R2P8_DualEL_ConversionOnly':{
        'hwy':[
            'PBA2050_RTP_ID_attributes',
            {'name':'EXP_uncommitted_noAllLaneTolling',     'kwargs':{'MODELYEAR':'2035','PATHWAY':"P5"},     'branch':'master'},
            'NGF_r2_DoubleEL',              # Double the express lanes if it's not already dual lanes
            'NGF_r2_HOV_to_EL',             # Convert HOV lanes to dual express lanes
        ],
        'trn':[
            'NGF_NoProject_farefiles',      # ensures these files get included; note this is not a real project
        ]
